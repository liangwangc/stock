...
function importWord($conn, $grade, $word, $order) {
    ...
    // 插入数据库（确保所有字段都是字符串）
    $word = (string)($wordInfo['word'] ?? '');
    $pronunciation = (string)($wordInfo['pronunciation'] ?? '');
    $meaning = (string)($wordInfo['meaning'] ?? '');
    
    // 处理example字段（可能是数组或字符串）
    $example = '';
...
echo "========================================\n";
echo "单词库批量导入工具（自动导入所有年级）\n";
echo "========================================\n\n";
$totalWords = 0;
foreach ($gradesToImport as $grade) {
    if (isset($wordLists[$grade])) {
        $totalWords += count($wordLists[$grade]);
    }
}

echo "将导入以下年级：" . implode(', ', $gradesToImport) . "\n";
echo "预计单词数量：{$totalWords} 个\n";
echo "预计耗时：" . round($totalWords * 0.2 / 60, 1) . " 分钟\n";
echo "\n";
echo "开始导入...\n\n";
...
foreach ($gradesToImport as $grade) {
    ...
    foreach ($words as $word) {
        ...
        if ($result->num_rows > 0) {
            $skipped++;
            if ($order % 10 == 0) {
                echo "  进度: {$order}/" . count($words) . " (跳过: {$skipped})\n";
            }
            $order++;
            continue;
        }
        
        echo "  [{$order}/" . count($words) . "] {$word}...";
    ...
    echo "\n{$gradeName} 导入完成：成功 {$success}，失败 {$failed}，跳过 {$skipped}\n\n";
