...
class Output(object):
    ...
    def hospital_patient_resultV2(self, df, e_value_df):
        """
        按医院、临床学科、医生统计O/E值，
        ...
        df['basy_yydj_str_str'] = df['basy_yydj']

        for i in Output.supplies_drug_dict.keys():
            df['basy_yydj'] = df['basy_yydj_str_str']
            # df = df.drop_duplicates('temp_id')
            ...
            if i in ['药品费', '总费用']:
                ...
                print("1", df_g.shape)
                df_g = pd.merge(left=df_g, right=e_value_df, on=['basy_yydj', 'jbbm', 'ssbm'], how='inner')
                print("2", df_g.shape)
            else:
                # 替换为模型即可
            ...
            # 更新cleaned_data中各类对应的oe值
            df = pd.merge(left=df, right=df_g[['temp_id', 'patient_odd_oe', 'nor_patient_odd_oe']], how='left', on='temp_id')
    ...
    def hospital_patient_result(self, df, e_value_df, min_cost):
        ...
        for i in Output.supplies_drug_dict.keys():
            ...
            if i in ['药品费', '总费用']:
                ...
                # 替换为模型即可
                print('before', df['basy_yydj'])
                df_g['level_{}_mean'.format(i)] = Model(i).predict(df)
                print('after', df['basy_yydj'])
