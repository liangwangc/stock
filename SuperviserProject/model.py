...
class Model(object):
    ...
    def __init__(self, label_type):
        self.basy_list = ["age", "basy_yydj", "basy_xb"]
        ...
        if self.label_type == "耗材费":
            self.feature_list = self.basy_list + self.disease_encode_list + self.haocai_disease_e_list + self.operation_encode_list + ["basy_sjzy"]
        elif self.label_type == "检查检验费":
            self.feature_list = self.basy_list + self.disease_encode_list + self.jcjy_disease_e_list + self.operation_encode_list + ["basy_sjzy"]

    def add_e_info(self, df):
        for i in range(3, 10, 1):
            temp = pd.read_csv("online_model/%s_%s_%s_1.csv" % (self.label_type, i, i), index_col=0, nrows=1)
            ...
            for column in temp.columns:
                if "E" in column:
                    ...
                    continue
            e_df = pd.read_csv("online_model/%s_%s_%s_1.csv" % (self.label_type, i, i), index_col=0, usecols=list(dtype_dict.keys()), dtype=dtype_dict)
            ...
            temp = pd.read_csv("online_model/%s_%s_%s_2.csv" % (self.label_type, i, "only_disease"), index_col=0, nrows=1)
            ...
            for column in temp.columns:
                if "E" in column:
                    ...
                    continue
            e_df = pd.read_csv("online_model/%s_%s_%s_2.csv" % (self.label_type, i, "only_disease"), index_col=0, usecols=list(dtype_dict.keys()), dtype=dtype_dict)
            df = pd.merge(df, e_df, how='left', on=["basy_zyzd_jbbm_for_str_%s" % i, "basy_yydj"], copy=False)
            print(df.shape)

            temp = pd.read_csv("online_model/%s_%s_%s_3.csv" % (self.label_type, "only_operation", i), index_col=0, nrows=1)
            ...
            for column in temp.columns:
                if "E" in column:
                    ...
                    continue
            e_df = pd.read_csv("online_model/%s_%s_%s_3.csv" % (self.label_type, "only_operation", i), index_col=0, usecols=list(dtype_dict.keys()), dtype=dtype_dict)
            df = pd.merge(df, e_df, how='left', on=["basy_yydj", "basy_ssjczbm1_for_str_%s" % i], copy=False)
            print(df.shape)
            # age

            temp = pd.read_csv("online_model/%s_%s_%s_4.csv" % (self.label_type, i, i), index_col=0, nrows=1)
            ...
            for column in temp.columns:
                if "E" in column:
                    ...
                    continue
            e_df = pd.read_csv("online_model/%s_%s_%s_4.csv" % (self.label_type, i, i), index_col=0, usecols=list(dtype_dict.keys()), dtype=dtype_dict)
            df = pd.merge(df, e_df, how='left', on=["basy_zyzd_jbbm_for_str_%s" % i, "basy_yydj", "basy_ssjczbm1_for_str_%s" % i, "age_group"], copy=False)
            print(df.shape)

            temp = pd.read_csv("online_model/%s_%s_%s_5.csv" % (self.label_type, i, "only_disease"), index_col=0, nrows=1)
            ...
            for column in temp.columns:
                if "E" in column:
                    ...
                    continue
            e_df = pd.read_csv("online_model/%s_%s_%s_5.csv" % (self.label_type, i, "only_disease"), index_col=0, usecols=list(dtype_dict.keys()), dtype=dtype_dict)
            df = pd.merge(df, e_df, how='left', on=["basy_zyzd_jbbm_for_str_%s" % i, "basy_yydj", "age_group"], copy=False)
            print(df.shape)

            temp = pd.read_csv("online_model/%s_%s_%s_6.csv" % (self.label_type, "only_operation", i), index_col=0, nrows=1)
            ...
            for column in temp.columns:
                if "E" in column:
                    ...
                    continue
            e_df = pd.read_csv("online_model/%s_%s_%s_6.csv" % (self.label_type, "only_operation", i), index_col=0, usecols=list(dtype_dict.keys()), dtype=dtype_dict)
    ...
    def predict(self, df):
        if "HAOCAI-3-3-E" not in df.columns:
            f = open('online_model/encode_for_category_dict.txt', 'r')
            encode_for_category_dict = json.loads(f.read())
            f.close()

            f = open('online_model/yydj_dict.txt', 'r')
        ...
        if self.label_type == "耗材费":
            lgb_model = lgbm.model_load("online_model/lgbm_model_haocai.model")
        elif self.label_type == "检查检验费":
            lgb_model = lgbm.model_load("online_model/lgbm_model_jcjy.model")
