...
class DataProcessing:
    ...
    def __init__(self, year, month, get_hos_info_in_year=2022):
        """
    ...
    def get_data(self):
        ...
        df_1.to_csv("data/original/hospital_info.csv")
        ...
        xy_col = """
                    sfzh, yljgid, bah, xb, csrq, rysj, cysj, cykb, sjzyts, zycs,
        ...
        # 主治医生zzys 改为 主诊医生zznys
        zy_col = """
                    sfzh, yljgid, bah, xb, csrq, rysj, cysj, cykb, sjzy, zycs,
                    ...
                    """
        sql_3 = """
                SELECT {} FROM zlk_raw.t_zyba WHERE substr(cysj, 1, 6) = '{}{}'
                """.format(zy_col, self.year, self.month)
        result_3 = db_table.query(sql_3)
    ...
    def clean_data(self):
        ...
        df = pd.read_csv("data/original/original_{}{}.csv".format(self.year, self.month), index_col=0,
                         dtype={'basy_cykb': str, 'basy_rysj': str, 'basy_cysj': str})
