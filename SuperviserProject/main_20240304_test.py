...
if __name__ == "__main__":
    ...
    province_hospital_df['basy_yljgid'] = province_hospital_df['basy_yljgid'].map(lambda a: "JG_" + str(a))
    province_hospital_set = set(province_hospital_df.loc[province_hospital_df['flag'] == 4, 'basy_yljgid'].astype(str).tolist())
    
    province_hospital_detail_df = cleaned_data[cleaned_data["basy_yljgid"].isin(province_hospital_set)]
    no_province_hospital_detail_df = cleaned_data[~cleaned_data["basy_yljgid"].isin(province_hospital_set)]
    
    no_province_hospital_detail_df = no_province_hospital_detail_df[~no_province_hospital_detail_df['basy_yydj'].isin(['未评级未评等', '一级甲等',
                                                                 '一级乙等', '一级未评等'])]
    cleaned_data = pd.concat([province_hospital_detail_df, no_province_hospital_detail_df])
    print("cleaned_data2", cleaned_data.shape)                                              
