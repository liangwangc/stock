    ...
    try:
        ...
        for table in tables_to_drop:
            try:
                ...
                print(f"✅ 删除表 {table} 成功", flush=True)
            except Exception as e:
                print(f"⚠️  删除表 {table} 时出现问题: {e}", flush=True)
        ...
        print("✅ 用户表创建成功", flush=True)
        
        # 创建部门表
        print("创建部门表...", flush=True)
        ...
        print("✅ 部门表创建成功", flush=True)
        
        # 创建档案分类表
        print("创建档案分类表...", flush=True)
        ...
        print("✅ 档案分类表创建成功", flush=True)
        
        # 创建稽核档案表
        print("创建稽核档案表...", flush=True)
        ...
        print("✅ 稽核档案表创建成功", flush=True)
        
        # 创建稽核日志表
        print("创建稽核日志表...", flush=True)
        create_logs_table = """
        ...
        print("✅ 稽核日志表创建成功", flush=True)
        
        # 创建通知表
        print("创建通知表...", flush=True)
        ...
        print("✅ 部门数据插入成功", flush=True)
        
        # 插入档案分类
        insert_categories = """
        INSERT INTO file_categories (name, code, description, sort_order) VALUES 
        ...
        try:
            ...
            if result["success"]:
                print("✅ 管理员用户创建成功！", flush=True)
                print("   用户名: admin", flush=True)
                print("   密码: admin123", flush=True)
                print("   角色: admin", flush=True)
            else:
                print(f"❌ 管理员用户创建失败: {result['message']}", flush=True)
                
        except Exception as e:
            print(f"❌ 创建管理员用户失败: {e}", flush=True)
            import traceback; traceback.print_exc()
        
        print("\n🎉 所有表重新创建完成！", flush=True)
        return db_manager
        
    except Exception as e:
        print(f"❌ 重新创建表失败: {e}", flush=True)
        import traceback; traceback.print_exc()
        return None
...
def main():
    """主函数"""
    print("=" * 60, flush=True)
    print("🏛️  稽核档案管理系统 - 重新创建表", flush=True)
    print("=" * 60, flush=True)
    
    # 重新创建表
    db_manager = recreate_tables()
    if not db_manager:
        print("❌ 表重新创建失败，系统无法继续", flush=True)
        sys.exit(1)
    
    print("\n" + "=" * 60, flush=True)
    print("🎉 系统重新初始化完成！", flush=True)
    print("=" * 60, flush=True)
