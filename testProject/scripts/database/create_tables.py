...
def create_tables():
    ...
    try:
        ...
        # 插入默认数据
        print("插入默认数据...", flush=True)
        
        # 插入部门
        insert_departments = """
        INSERT IGNORE INTO departments (name, code, description, sort_order) VALUES 
        ('稽核部', 'AUDIT', '负责内部稽核工作', 1),
        ('财务部', 'FINANCE', '负责财务管理', 2),
        ('人事部', 'HR', '负责人力资源管理', 3),
        ('技术部', 'TECH', '负责技术开发', 4),
        ('运营部', 'OPERATIONS', '负责日常运营', 5)
        """
        ...
        print("\n🎉 所有数据库表创建完成！", flush=True)
        return db_manager
        
    except Exception as e:
        print(f"❌ 创建表失败: {e}", flush=True)
...
def main():
    """主函数"""
    print("=" * 60, flush=True)
    print("🏛️  稽核档案管理系统 - 数据库表创建", flush=True)
    print("=" * 60, flush=True)
    
    # 创建表
    db_manager = create_tables()
    if not db_manager:
        print("❌ 数据库表创建失败，系统无法继续", flush=True)
        sys.exit(1)
