...
def logs():
    ...
    try:
        from audit_system.database import AuditLogDAO, UserDAO
        log_dao = AuditLogDAO(db_manager)
        user_dao = UserDAO(db_manager)
        ...
        return render_template('logs.html', logs=[], users=[])
...
def get_log_detail(log_id):
    ...
    try:
        ...
        # 获取日志详情
        log_detail = log_dao.get_log_by_id(log_id)
        
        if not log_detail:
            return jsonify({'success': False, 'message': '日志不存在'}), 404
        
        return jsonify({'success': True, 'log': log_detail})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
