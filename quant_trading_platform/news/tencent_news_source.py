...
class TencentNewsSource(BaseNewsSource):
    """腾讯财经新闻源"""
    ...
    def get_stock_news(self, symbol: str, limit: int = 10) -> List[Dict]:
        ...
        urls_to_try = [
            ...
            f"https://finance.qq.com/stock/{symbol}/",  # 备用URL 2
        ]
                    ...
                    for item in news_items[:limit * 2]:
                        try:
                            ...
                            if not link.startswith('http'):
                                if link.startswith('/'):
                                    link = 'https://finance.qq.com' + link
                                else:
                                    link = self.base_url + '/' + link
                            
                            time_elem = item.find(['span', 'time', 'em'], class_=re.compile(r'time|date|pub', re.I))
        ...
        self.logger.info(f"从腾讯财经获取到 {len(news_list)} 条新闻")
    ...
    def get_market_news(self, limit: int = 20) -> List[Dict]:
        ...
        urls_to_try = [
            ...
            f"{self.base_url}/",  # 备用URL 1（首页）
            f"{self.base_url}/news/",  # 备用URL 2
            f"https://finance.qq.com/stock/",  # 备用URL 3
        ]
        ...
        self.logger.info(f"从腾讯财经获取到 {len(news_list)} 条市场新闻")
        return news_list[:limit]
